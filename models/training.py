...
import models.model as wrapper


def SaveModel(model,
              name,
              pathmodel='./models/Sessions/'):
    ...
    model.save(filepath)
    model.save_weights(pathmodel + name + '_weight_' + date + '.h5')
...
def LoadModel(tf_mapping):
    ...
    filename = filedialog.askopenfilename(
            initialdir = './Sessions/',
            filetypes = (("Template files", "*.h5"), ("All files", "*"))
            )
    model = load_model(
            filename,
            custom_objects={
                'tonnetz':wrapper.wrap_loss_function(tf_mapping=tf_mapping),
                'euclidian':wrapper.wrap_loss_function(tf_mapping=tf_mapping)
                }
            )
